              ...
              >
                ...
                <el-table-column>
                  <template slot-scope="test">
                    ...
                    <base-button
                      type="danger"
                      @click="deleteHandler(test.$index, test.row)"
                      >Delete</base-button
                    >
                  </template>
                </el-table-column>
...
export default {
  ...
  data() {
    return {
      propsToSearch: [["_id"], ["compName"],["compType"],["compAddress"],["compTlp"],["provDetails", "provName"],["kabkotDetails", "kabkotName"]],
      searchQuery: "",
      selected: {
        jenisIndustri: null,
        prov: null,
  ...
  computed: {
    csvData() {
      ...
      for (let i = 0; i < tempData.length; i++) {
        formattedData.push({
          No: z,
          "Nama Industri": tempData[i].compName,
          "Jenis Industri": tempData[i].compType,
          // "Waktu Pendaftaran": moment
          //   .unix(tempData[i].detail.createdAt)
          //   .calendar(),
         
          "Alamat Perusahaan": tempData[i].compAddress,
  ...
  methods: {
     async changeFilter(event,type){
      ...
      if(type =="jenis_industri"){
        ...
        this.validasi = "";
        if(event !=""){
          this.tableData = this.tableData.filter(x => x.compType === event)
    ...
    myDownloadCSV(data) {
      ...
      a.setAttribute("download", "ListIndustri.csv");
    ...
    deleteHandler(id, row) {
      ...
      Axios.delete(`${defaults.baseURL}/comp/${row._id}`).then((res) => {
        Axios.get(`${defaults.baseURL}/comp/list`, {
          headers: { token: this.$store.state.token },
        }).then((res) => {
    ...
    exportExcel() {
      window.open(`${defaults.baseURL}/comp/list-export`, "_blank");
    },
  },
  created() {
    this.getListIndustri();
