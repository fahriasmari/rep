  ...
  <div>
    <div id="listed-report">
      ...
      >
        ...
        <h4 class="text-default m-0">List Database</h4>
      </div>
          ...
          <div class="col-lg-12">
            ...
            <div class="card">
              ...
              >
                ...
                <el-table-column
                  label="Waste Source"
                  prop="companyDetails.compWasteSource"
                  min-width
                  align="left"
                  sortable
                ></el-table-column>
                ...
                <!-- <el-table-column>
                  <template slot-scope="test">
                    <base-button type="primary" @click="edit(test.$index, test.row)">Edit</base-button>
...
export default {
  ...
  data() {
    return {
      propsToSearch: [["_id"], ["companyDetails", "compName"],["companyDetails", "compType"],["companyDetails", "compTlp"],["provDetails", "provName"],["kabkotDetails", "kabkotName"]],
      selected: {
        jenisIndustri: null,
        prov: null,
        kabkot: null,
  ...
  methods: {
      ...
      if(type =="jenis_industri"){
        ...
        this.validasi = "";
        if(event !=""){
          this.tableData = this.tableData.filter(x => x.compName === event)
          this.filter_data.count = this.tableData.length
          this.filter_data.status = true
        }
        else {
        ...
        if(event !=""){
          this.tableData = this.tableData.filter(x => x.provDetails.provName === event)
          this.filter_data.count = this.tableData.length
          this.filter_data.status = true
        }
        else {
        ...
        this.validasi = "";
        if(event !=""){
          this.tableData = this.tableData.filter(x => x.kabkotDetails.kabkotName === event)
          this.filter_data.count = this.tableData.length
          this.filter_data.status = true
        }
        else {
    ...
    myDownloadCSV(data) {
      ...
      a.setAttribute("download", "ListLogger.csv");
    ...
    edit(id, row) {
      const payload = {
        ...
        compPermit: row.compPermit,
        compWasteSource: row.compWasteSource,
        compInstance: row.compInstance,
        compPermitDate: row.compPermitDate,
        compTech: row.compTech,
      };
      this.$store.commit("setCompEditData", payload);
      this.$router.push("/admin/industriform");
    },
    ...
    getListLogger(){
      axios
      .get(`${defaults.baseURL}/logger/list`, {
        headers: { token: this.$store.state.token },
      })
      .then((res) => {
    ...
    exportExcel() {
      window.open(`${defaults.baseURL}/logger/list-export`, "_blank");
    },
  },
  created() {
    this.getListLogger();
