      ...
      <form class="col-12 px-4 border-top pt-4">
        ...
        <div class="form-group row">
          ...
          <div class="col-lg-10">
            ...
            <small
              v-if="form.lampiranBeritaAcara.upload == 2 || form.lampiranBeritaAcara.path"
...
export default {
  ...
  data() {
    return {
      ...
      originalCity: [],
      form: {
        tanggalBeritaAcara: this.data.tanggalBeritaAcara,
        tanggalKunjungan: this.data.tanggalKunjungan,
        isRegistered: !!this.data.compID,
        compID: this.data.compID,
        detailOtherComp: {
          compName: this.data.compName
        },
        provID: this.data.provID,
        kabkotID: this.data.kabkotID,
        lampiranBeritaAcara: {
          path: this.data.lampiranBeritaAcara,
          upload: 0,
          progress: 0,
        },
      },
  ...
  methods: {
    ...
    getCompanyData() {
      const company = this.companies.find((company) => company.compID === this.form.compID);
    ...
    async onSubmit() {
      ...
      if (!tempValidate) {
        ...
        const data = await axios.put(`${defaults.baseURL}/beritaacara/${this.data.id}`, this.form);
        if (data.status === 200) {
          Swal.fire({
            icon: 'success',
            title: 'Tersimpan',
            text: `Data telah rersimpan`,
          }).then(() => {
            this.$emit("close");
          });
        } else {
    ...
    getCity() {
      this.cities = this.originalCity.filter((city) => city.provID === this.form.provID);
    },
    async getRegion() {
  ...
  mounted() {
    this.$refs.tanggalKunjungan.fp.setDate(this.data.tanggalKunjungan);
    this.$refs.tanggalBeritaAcara.fp.setDate(this.data.tanggalBeritaAcara);
  },
};
</script>