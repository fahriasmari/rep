...
const flatPickerConfig = {
  ...
  //     if (dates.length == 2) {
  //         this.startDate = moment(dates[0]).unix();
  //         this.endDate = moment(dates[1]).unix();
...
export default {
  ...
  data() {
    return {
      ...
      cityData: [],
      selectedRows: []
      // startDate :moment().startOf('days').unix(),
      // endDate:moment().endOf('days').unix()
    };
  },
  computed: {
    apiParams() {
      return {
        ...
        ...(this.filter.date && {
          end: +this.filter.date.split(" to ")[1]
        })
      };
    }
  },
  methods: {
    async getData() {
      ...
      const axiosConfig = {
        url: `/report/hourly/${this.auth._id}`,
        methods: "get",
      ...
      swalWithBootstrapButtons
        ...
        .then(async result => {
          if (result.value) {
            ...
            window.open(
              `${defaults.baseURL}/report/hourly-export/${this.auth._id}?${params}`,
              "_blank"
            );
