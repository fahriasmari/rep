  ...
  <div>
    <div id="daily-report">
      ...
      >
        ...
        <h4 class="text-default m-0">Data Perhari</h4>
      </div>
          ...
          <div class="col">
            <div class="card">
              ...
              >
                ...
                <el-table-column label="Debit (m3/hari)">
                  <template v-slot="{ row }">
                    {{ setBmal(row.dataParam[4].value) }}
                  </template>
                </el-table-column>
                <el-table-column label="Beban COD (kg/hari)">
                  <template v-slot="{ row }">
                    {{
                      setBebanBmal(
                ...
                <el-table-column label="Beban TSS (kg/hari)">
                  <template v-slot="{ row }">
                    {{
                      setBebanBmal(
                ...
                <el-table-column label="Beban NH3N (kg/hari)">
                  <template v-slot="{ row }">
                    {{
                      setBebanBmal(
...
export default {
  ...
  methods: {
    async getData() {
      ...
      const axiosConfig = {
        url: `/report/daily/${this.auth._id}`,
        methods: "get",
    ...
    async getFilterData() {
      const { data } = await axios.get(
        `${defaults.baseURL}/report/list/${this.auth._id}`
      );
      ...
      swalWithBootstrapButtons
        ...
        .then(async result => {
          if (result.value) {
            ...
            window.open(
              `${defaults.baseURL}/report/daily-export/${this.auth._id}?${params}`,
              "_blank"
            );
