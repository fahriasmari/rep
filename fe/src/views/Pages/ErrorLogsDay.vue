            ...
            <div class="card">
              ...
              <div class="p-3 w-100">
                ...
                <div class="row" v-if="this.$store.state.auth.level < 1">
                  ...
                  <div class="col-2">
                    <base-button class="w-100" size="md" type="primary" @click="exportExcel">Download</base-button>
                    <!-- <base-button
                      class="w-100"
                      size="md"
                      type="primary"
...
export default {
  ...
  watch: {
    selectedTime() {
      ...
      this.queryArray[4] = tempStart;
      this.queryArray[5] = tempEnd;
      this.generateData();
    },
  },
  methods: {
    ...
    generateData() {
      ...
      const params = `type=${tempArray[0]}&prov=${tempArray[1]}&kabkot=${tempArray[2]}&comp=${tempArray[3]}&start=${tempArray[4]}&end=${tempArray[5]}`;
      axios
        .get(`${defaults.baseURL}/error/recapDay/${auth._id}?${params}`, {
          headers: { token: this.$store.state.token },
        })
        .then((res) => {
    ...
    exportExcel() {
      ...
      const auth = JSON.parse(localStorage.getItem("auth-data"));
      const params = `type=${tempArray[0]}&prov=${tempArray[1]}&kabkot=${tempArray[2]}&comp=${tempArray[3]}&start=${tempArray[4]}&end=${tempArray[5]}`;
      window.open(`${defaults.baseURL}/error/recapDay-export/${auth._id}?${params}`, "_blank");
    },
      ...
      swalWithBootstrapButtons
        ...
        .then((result) => {
          if (result.value) {
            ...
            let titleText =
              "Laporan Early Warning (1 Jam) - " + this.$store.state.auth.name;
