            ...
            <div class="card-footer" style="background-color: #ff9999">
              <div class="row">
                <div class="col text-right">
                  <base-button
                    @click="$router.push('/content/errorlogs')"
            ...
            <div :key="item.id" class="col-1-5">
              <div class="card m-0">
                ...
                <h2
                  ...
                  style="height: 5rem"
                >
                  <span class="mr-2">{{ item.value.toFixed(2) }}</span>
                  <span>{{ item.unit }}</span>
                </h2>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    getErrorDataHourly() {
      ...
      const authData = JSON.parse(localStorage.getItem("auth-data"));
      axios
        .get(`${defaults.baseURL}/error/recap/${authData._id}`)
        .then((res) => {
    ...
    getErrorData() {
      ...
      const authData = JSON.parse(localStorage.getItem("auth-data"));
      axios
        .get(`${defaults.baseURL}/error/list-recent/${authData._id}`)
        .then((res) => {
    ...
    insertParam(idData) {
      const authData = JSON.parse(localStorage.getItem("auth-data"));
      axios
        .get(`${defaults.baseURL}/pj/data/${authData._id}`, {
          headers: { token: this.$store.state.token },
        })
        .then((res) => {
    ...
    insertParamClickMarker(idData, role) {
      ...
      axios
        .get(
          `${defaults.baseURL}/pj/data/${authData._id}?filter=${idData}&type=${role}`
        )
        .then((res) => {
          // console.log(res.data);
